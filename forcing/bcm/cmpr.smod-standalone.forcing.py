...
for i,j,k in itertools.product(np.arange(volr.shape[0]),np.arange(volr.shape[1]),np.arange(volr.shape[2])):
    ...
    rhpkd = rh_rg[i,j,k]
    if ( rhpkd >= 90 ):
        rhpkdind = np.where( (rh < rhpkd + 0.5) & (rh >= rhpkd - 0.5) )[0].item()
    elif ( rhpkd >= 80 and rhpkd <= 90 ):
        rhpkdind = np.where( (rh < rhpkd + 1) & (rh >= rhpkd - 1) )[0].item()
    elif ( rhpkd > 79.5 and rhpkd < 80 ):
        rhpkdind = np.where( rh == 80 )
    elif ( rhpkd < 30 ):
        rhpkdind = np.where( rh == 30 )
    else:
        rhpkdind = np.where( (rh < rhpkd + 2.5) & (rh >= rhpkd - 2.5) )[0].item()

    mac_cs[i,j,k] = mac_cs_t[wlpkdind,volcspkdind,rhpkdind]
    mac_em[i,j,k] = mac_em_t[wlpkdind,volempkdind,rhpkdind]
    mscb_cs[i,j,k] = mscb_cs_t[wlpkdind,volcspkdind,rhpkdind]
    mscb_em[i,j,k] = mscb_em_t[wlpkdind,volempkdind,rhpkdind]
    mec_cs[i,j,k] = mac_cs_t[wlpkdind,volcspkdind,rhpkdind] + msc_cs_t[wlpkdind,volcspkdind,rhpkdind]
    mec_em[i,j,k] = mac_em_t[wlpkdind,volempkdind,rhpkdind] + msc_em_t[wlpkdind,volempkdind,rhpkdind]
...
# ignore MSC
#drf_sfc_smod_mon = f0 * ta * (1. - rs) * 2. * bcso4_col * dmac
#drf_toa_smod_mon = f0 * np.power(ta,2) * 2. * rs * 2. * bcso4_col * dmac
